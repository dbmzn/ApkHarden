...
- [ ] Select online APK, candidate APK, metadata, keystore, and output directory.
- [ ] Accept alias and masked passwords without persistence or logging.
- [ ] Show status plus every finding code, level, message, and detail.
- [ ] Support explicit approval for `REQUIRES_APPROVAL` findings.
- [ ] Disable export until all inputs are valid and analysis is statically verified.
- [ ] Clear password state after every analyze/export attempt.
- [ ] State clearly that `STATIC_VERIFIED` is not device or release qualification.

## Task 4: Separate the legacy experimental workflow

- [ ] Rename the existing navigation item to make its experimental status obvious.
- [ ] Display `实验模式，不允许用于正式发布` in the whole-DEX screen.
- [ ] Keep the existing hardening and privacy-scanner behavior working.

## Task 5: Verify Phase 4

- [ ] Run release-core tests, desktop tests, and `validatePlugins`.
- [ ] Verify a valid fixture exports a complete signed bundle.
- [ ] Verify a blocked fixture exports nothing.
- [ ] Verify JSON and HTML are derived from the same assessment.
- [ ] Verify no password appears in logs, reports, or committed fixtures.
