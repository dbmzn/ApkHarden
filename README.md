...
| 分区 | 功能 | 主要用途 |
|---|---|---|
| 设备调试 | ADB 工具箱（默认入口） | USB / Wi-Fi 连接、手机剪贴板、截图录屏、镜像、日志和故障诊断 |
...
桌面端启动直接进入“ADB 工具箱”，侧栏优先展示设备调试与安装验证。APK 工具区包含体检、Manifest、文件浏览、版本对比、包体积分析和加固，配置区提供签名百宝箱。进入“APK 工具 → APK 加固”后只需选择输入 APK 和输出路径，已保存的正式签名会被自动复用。点击“开始加固并签名”后，输出目录会得到：
...
应用默认打开“设备调试 → ADB 工具箱”。连接真机或模拟器并开启 USB 调试，或通过右上角“无线连接”接入手机后，选择在线设备即可使用：
