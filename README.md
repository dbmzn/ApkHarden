...
一个以 ADB 日常调试为主入口的 Android 设备与 APK 工具箱（Compose Desktop GUI）。启动即进入 ADB 工具箱，支持 USB / Wi-Fi 连接、截图录屏、实时镜像、手机剪贴板和日志诊断；同时提供 APK 体检、版本对比、包体分析、签名及加固等辅助工具。
...
- **ADB 工具箱**：提供按当前桌面可用区域最大化适配的独立实时镜像窗口与鼠标/键盘控制、日志过滤、截图预览/复制/保存、录屏、性能快照、Intent / Deep Link 调试、崩溃与 ANR 采集包、权限授予、应用数据清理、进程和页面栈查看；截图与录屏自动使用时间命名，缺少系统 `screenrecord` 的设备会自动切换到随桌面版部署的 scrcpy 高帧率录制，连续截图编码仅作为最后保底，诊断包会把设备、应用、CPU、内存、页面栈、Crash buffer、last ANR 与 DropBox 信息打包到 `Downloads`，清数据操作会二次确认
...
| 分区 | 功能 | 主要用途 |
|---|---|---|
| 常用工具 | 截图与录屏（默认入口）、实时镜像、手机剪贴板、文件互传 | 四个独立侧栏入口，共用设备选择与 USB / Wi-Fi 连接 |
| 常用工具 | 二维码与条形码 | 本地生成二维码、Code 128、EAN-13、EAN-8、Code 39、ITF；二维码可添加中心图片，支持替换、移除和三档大小，使用高纠错并校验合成结果；保存 PNG 或复制码图，从图片文件或剪贴板识别并复制内容，无需连接手机 |
| 设备调试 | 更多调试 | 日志、性能、Intent、应用操作和故障诊断 |
| 设备调试 | 设备安装验证 | 覆盖安装并执行冷启动、进程、前台 Activity 和闪退日志门禁 |
...
| APK 工具 | APK 加固 | DEX 加密、壳注入、16KB ZIP 对齐、正式签名和加固报告 |
...
./gradlew run
```
启动后即可连接设备使用 ADB 工具。需要加固签名时，先进入“签名百宝箱”，选择 keystore（`.jks` / `.p12`），填写别名和密码，点击“验证并保存”。Windows 上签名密码通过 DPAPI 加密；macOS 上通过 AES-GCM 加密，加密密钥保存在当前用户的钥匙串中。密码不会以明文写入配置。

桌面端启动直接进入“截图与录屏”，侧栏“常用工具”独立展示截图录屏、实时镜像、手机剪贴板和文件互传；“设备调试”包含更多调试与安装验证。APK 工具区包含体检、Manifest、文件浏览、版本对比、包体积分析和加固，配置区提供签名百宝箱。进入“APK 工具 → APK 加固”后只需选择输入 APK 和输出路径，已保存的正式签名会被自动复用。点击“开始加固并签名”后，输出目录会得到：
...
从侧栏“常用工具”直接进入截图录屏、镜像、剪贴板或文件互传；其他功能在“设备调试 → 更多调试”。连接真机或模拟器并开启 USB 调试，或通过右上角“无线连接”接入手机后，选择在线设备即可使用：

| 工具 | 操作与结果 |
|---|---|
| 实时镜像 | 读取设备当前旋转后的显示边界，按显示器 DPI 换算物理工作区并等比铺满一屏；大尺寸平板在启动前将视频最大边限制为 1920，避免硬件编码器先失败再重试；支持点击、滑动、键盘输入、右键返回和 Ctrl 快捷键，切换 ADB 工具标签保持连接，离开 ADB 工具箱自动断开 |
| 日志过滤 | 可按包名限定当前进程，并按关键字过滤最近 800 行日志 |
| 设备截图 | 截图后在当前应用中预览；可复制到剪贴板或保存到 `Downloads` |
| 手机剪贴板 | ADB 工具箱选择在线设备，打开“手机剪贴板”后每秒读取文字和链接，可选中或一键复制到电脑；离开页面停止接收，不保存历史 |
| 文件互传 | 聊天式文件传输助手：右侧展示发送记录，左侧展示接收记录，底部选择文件发送；“浏览手机文件”打开独立目录面板并接收文件或选择目标目录。会话记录按设备隔离，在常用工具间切换时保留，离开设备工具或关闭应用后清除；同名文件不覆盖，一次传输一个文件，离开文件互传页会取消正在进行的传输 |
| 无线 ADB | ADB 工具箱右上角“无线连接”：同一 Wi-Fi 下支持设备发现、Android 11+ 配对码配对、手动连接/断开；旧手机先通过 USB 开启 TCP 5555，连接后可拔线，用完可关闭旧版无线调试。Android 17 配合 Platform-Tools 37+ 使用 ADB Wi-Fi 2.0；旧设备不具备完整 2.0 能力 |
